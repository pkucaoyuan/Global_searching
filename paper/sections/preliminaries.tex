...
% THEORY-SETTING-BEGIN{diffusion-models}
...
builds on the \emph{score} (not to be confused with the verifier score
in \cref{def:verifier}),
defined as $s(\mathbf{x}) = \nabla_\mathbf{x} \log q(\mathbf{x})$,
...
% THEORY-SETTING-END{diffusion-models}
...
The core difficulty is sequential: the noise $\varepsilon_t$ at timestep $t$
shapes the latent $x_{t-1}$, which in turn determines which noises are
effective at subsequent steps.
This sequential dependence distinguishes the problem from a standard
resource-allocation formulation and motivates the two-level decomposition
introduced in \cref{sec:framework}.
...
through the denoising network, the dominant cost in diffusion sampling.
...
optimization~\citep{chen2000simulation}.
\end{definition}
