...
All experiments run on a single NVIDIA A100 GPU; wall-clock time is dominated by forward passes through the denoising network and therefore determined by the NFE budget.
...
GAINS improvements widen with budget, and GAINS at 400 NFE already surpasses uniform allocation at 500--800 NFE.}
\label{tab:sd_budget}
\begin{tabular}{c|c|c}
\toprule
...
The GAINS 400-NFE result ($0.7248$) already outperforms uniform allocation at
...
GAINS at \textbf{400 NFE} ($0.8946$) already overtakes uniform allocation
at \textbf{800 NFE} ($0.8892$), achieving higher compressibility with \textbf{50\% fewer} function evaluations.
...
Consistent with \cref{prop:offline}(iii), the non-uniform gains widen as sensitivity dispersion increases: SD's early-timestep concentration yields moderate dispersion and steady improvements, while the stagnation of uniform allocation on compressibility reflects its inability to exploit this structure.
...
EDM's concentrated mid-trajectory sensitivity amplifies GAINS, which at 144 NFE already exceeds uniform allocation at 288 NFE for Brightness.}
...
The GAINS 144-NFE brightness ($0.9887$) already outperforms uniform allocation at
288 NFE ($0.9787$), delivering higher quality with \textbf{50\% fewer}
function evaluations.
Similarly, at 180 NFE GAINS ($0.9904$) surpasses uniform allocation at 288 NFE,
...
The GAINS 144-NFE result ($0.6845$) already beats uniform allocation at 180 NFE
($0.6774$), saving \textbf{20\%} NFE;
the GAINS 180-NFE result ($0.7003$) exceeds uniform allocation at 288 NFE ($0.6901$),
saving \textbf{37.5\%} NFE.
The improvements grow from +0.0131 to +0.0229 and
+0.0264 as budget increases, confirming that global scheduling directs
incremental NFE to productive timesteps rather than spreading it uniformly.
...
Online control contributes incremental gains beyond offline allocation alone on both metrics.}
...
The incremental gain from online control over offline-only (e.g., $+0.0090$ on Brightness, $+0.0073$ on Compressibility) provides empirical evidence of the Jensen gap predicted by \cref{prop:online}(i): because per-prompt sensitivity profiles $\boldsymbol{\sigma}$ vary, a fixed allocation tuned to the population mean leaves recoverable value on the table.
...
Naive corresponds to the uniform allocation method; GAINS is the proposed global scheduler.
Improvements persist across a diverse prompt set, confirming that the benefit reflects systematic reallocation rather than prompt-specific artifacts.}
\label{tab:larger_prompt_exp}
\begin{tabular}{lcc}
\toprule
 & \textbf{brightness} & \textbf{compressibility} \\
\midrule
Naive             & $0.6949 \pm 0.0756$ & $0.7938 \pm 0.0892$ \\
...
GAINS improves both metrics under every local operator, confirming that global scheduling composes with the local search mechanism.}
...
\cref{tab:local_search_combo} confirms this: under both \textbf{Zero-order} (local perturbation, \cref{ex:local-perturbation}) and \textbf{Random} (i.i.d.\ noise draws, \cref{ex:random-search}) local search, replacing uniform allocation with GAINS improves \textit{Brightness} and \textit{Compressibility}.
...
Margins are smaller than on SD and EDM, consistent with the narrower variance dispersion after ODE-to-SDE conversion.}
...
and +0.0036;
the GAINS 100-NFE score ($0.8170$) surpasses uniform allocation at 150 NFE
...
and GAINS also reduces run-to-run standard deviations (e.g., from $0.0058$ to $0.0046$ for compressibility at 150~NFE).
