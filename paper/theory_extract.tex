...
% Stub section from algorithm.tex
\subsection{(See main paper)} \label{sec:scheduling-optimality}
% Stub: tab:phi-summary from general_local_search.tex
...
\paragraph{Diffusion Model Formulations.}
...
$\tilde{\mathbf{x}}_t = \mathbf{x}_0 + \varsigma_t\boldsymbol{\varepsilon}$
for $0 < \varsigma_1 < \varsigma_2 < \cdots < \varsigma_T$.
A neural network
$\mathbf{s}_\theta(\tilde{\mathbf{x}}_t, \varsigma_t)$ approximates
...
\begin{remark}
The noise trajectory search problem is a sequential budget-allocation
problem: the noise~$\varepsilon_t$ at timestep~$t$ shapes the
latent~$x_{t-1}$, which in turn determines which noises are effective
at subsequent steps.
This sequential dependence distinguishes the problem from a standard
resource-allocation formulation and motivates the two-level
decomposition introduced in \cref{sec:framework}.
\end{remark}
...
\begin{remark}
The verifier is treated as a black box throughout; our results hold
for any bounded, query-efficient quality measure.
In operations-research applications, the verifier could be a
simulation-fidelity metric, a constraint-satisfaction indicator, or a
...
\begin{remark}
The NFE budget is analogous to a computational budget in simulation
optimization: each function evaluation corresponds to one forward pass
through the denoising network, which is the dominant cost in diffusion
sampling.
Standard sampling consumes exactly~$T$ evaluations; noise trajectory
search allocates the remaining~$B - T$ evaluations to local
refinement.
\end{remark}
...
\paragraph{Reverse SDE and Score Function.}
...
timesteps, where the verifier landscape is rougher, and smaller at
late timesteps, where denoised predictions are nearly converged.
\end{remark}
...
\paragraph{Abstract Local Search and Gain Definition.}
